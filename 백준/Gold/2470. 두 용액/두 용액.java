...
public class Main {
    ...
    static void pro() {
        // A에 대해 이분 탐색을 할 예정이니까 정렬을 미리 함
        Arrays.sort(A, 1, N + 1);
        
        int best_sum = Integer.MAX_VALUE;
        int v1 = 0, v2 = 0;
        for(int left = 1; left <= N - 1; left++) {
            // A[left] 용액을 쓸 것이므로 -A[left]와 가장 가까운 용액을 자신의 오른쪽 구간에서 찾을 것
            int res = lower_bound(A, left + 1, N, -A[left]);
            
            // A[res - 1]와 A[res] 중에 A[left]와 섞엇을 때 정보를 정답에 갱신시킨다.
